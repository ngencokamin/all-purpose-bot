...
class Callbacks(commands.Cog):
    def __init__(self, bot: commands.Bot):
        self.bot = bot
        self.db = DB('db/db.sqlite', tablename='callbacks')
        self.plugins = DB('db/db.sqlite', tablename='plugins')
        super().__init__()

    async def add_callback(self, interaction, trigger: str, response: str):
        callbacks = self.db[interaction.guild.id]
        if not callbacks.get("triggers"):
            callbacks["triggers"] = []
        if not callbacks.get("channels"):
            callbacks["channels"] = []
    ...
    async def list_callbacks(self, interaction):
        callbacks = self.db[interaction.guild.id]
        embedVar = discord.Embed(title="Callbacks", description="Callback triggers and values")
    ...
    async def remove_callback(self, interaction, trigger: str):
        callbacks = self.db[interaction.guild.id]
...
async def setup(bot: commands.Bot):
    await bot.add_cog(Callbacks(bot))
